...
public class Actions
{
	...
	public void checkUpdateStatus(String us){
	    ...
	    fw = lr.eval_string(""+lr.eval_string("<fwver>")+"");
	    if (code.equals("0")) {
		if (fw == null) {
		    lr.end_transaction("FW001",lr.FAIL);
		...
		web.disable_keep_alive (); 
		web.reg_save_param("codec",new String[]{"LB="+lr.eval_string("<statusCode>")+"","RB=</statusCode>","LAST"});
		//web.reg_save_param("percent",new String[]{"LB="+lr.eval_string("<downloadPercentage>")+"","RB=</downloadPercentage>","LAST"});
		web.reg_save_param("updatestatus",new String[]{"LB="+lr.eval_string("<updateSuccess>")+"","RB=</updateSuccess>","LAST"});//0为成功、1为失败、2为升级中
		web.add_header("Authorization","Digest userId="+lr.eval_string("<userid>")+",qId="+lr.eval_string("<qid>")+",response="+lr.eval_string("<resp>")+"");//digest 
	    ...
	    code = lr.eval_string(""+lr.eval_string("<codec>")+"");
	    us = lr.eval_string(""+lr.eval_string("<updatestatus>")+"");
	    if (code.equals("0") ) {
		//if (us.equals("2")) {
