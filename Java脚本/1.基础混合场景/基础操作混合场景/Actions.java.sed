...
public class Actions
{
	...
	private void queryIPCRequest(String[] s){
	    ...
	    time = lr.eval_string(""+lr.eval_string("<nowTime>")+"");
	    //System.out.print(time);
	    lr.save_string(time,"thisTime");
	    try	{
		lr.save_string(AESUtil.getEncodeQKey(lr.eval_string(""+lr.eval_string("<mdqkey>")+""),time),"encodemdqKey");
	    ...
	    xmlBody = "Body=<appVerifyDevice version=\"1.0\" xmlns=\"urn:skylight\">"
		    ...
		    + "\r\n"+lr.eval_string("<dateTime>")+""+lr.eval_string("<thisTime>")+"</dateTime>"
		    + "\r\n"+lr.eval_string("<encodeQKey>")+""+lr.eval_string("<encodemdqKey>")+"</encodeQKey>"
		    + "\r\n</appVerifyDevice>\r\n";
	    lr.save_string(xmlBody,"xmlbody");
	    //System.out.println(xmlBody);
	    try {
	...
	private void ownedByOtherRequest(String[] s){
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/app/device/"+lr.eval_string("<qid>")+"/ownedByOther","uri");
	    lr.save_string("GET","method");
	    //System.out.println(xmlBody);
		...
		lr.output_message("Success code: " + code);
		lr.output_message("isOwnedByOther: " + lr.eval_string(""+lr.eval_string("<owner>")+""));
	    }
	    else{
		lr.end_transaction("AU023",lr.FAIL);
                lr.error_message("Error Code:" + code);
	...
	private void isLiveIPCRequest(String[] s){
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/app/device/"+lr.eval_string("<qid>")+"/isLiveDevice.app","uri");
	    lr.save_string("GET","method");
		...
		lr.output_message("Success code: " + code);
		lr.output_message("Live Or Not: " + lr.eval_string(""+lr.eval_string("<live>")+""));
	    }
	    else{
		lr.end_transaction("AU010",lr.FAIL);
                lr.error_message("Error Code:" + code);
	...
	private void deviceShareRequest(){
	    ...
	    lr.save_string("POST","method");
	    try {
		lr.save_string(HashUtil.getEncCheckSum("SHARE_DEVICE",""+lr.eval_string("<userID>")+"",""+lr.eval_string("<compID>")+""),"checks");
	    } catch ( Exception err ) {
		lr.message("An exception occurred: " + err.toString());
	    }
	    xmlBody = "Body=<?xml version=\"1.0\"?>"
		    + "\r\n"+lr.eval_string("<request>")+""
		    + "\r\n"+lr.eval_string("<userId_master>")+""+lr.eval_string("<userID>")+"</userId_master>"
		    + "\r\n"+lr.eval_string("<userId_share>")+"666666</userId_share>"
		    + "\r\n"+lr.eval_string("<appModel>")+""+lr.eval_string("<appM>")+"</appModel>"
		    + "\r\n"+lr.eval_string("<qId>")+""+lr.eval_string("<qid>")+"</qId>"
		    + "\r\n"+lr.eval_string("<model>")+""+lr.eval_string("<cameraM>")+"</model>"
		    + "\r\n"+lr.eval_string("<companyId>")+""+lr.eval_string("<compID>")+"</companyId>"
		    + "\r\n"+lr.eval_string("<checksum>")+""+lr.eval_string("<checks>")+"</checksum>"
		    + "\r\n</request>\r\n";
	    lr.save_string(xmlBody,"xmlbody");
	...
	private void queryIPCListRequest(String[] s){
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/app/device/"+lr.eval_string("<userid>")+"/query.app","uri");
	    lr.save_string("GET","method");
	    xmlBody = "Body=";
	    //System.out.println(xmlBody);
	    lr.save_string(xmlBody,"xmlbody");
	...
	private void modifyMotionRequest(String[] s){
	    ...
	    xmlBody = "Body=<motionSensitivity version=\"1.0\" xmlns=\"urn:skylight\">"
		    + "\r\n"+lr.eval_string("<qId>")+""+lr.eval_string("<qid>")+"</qId>"
		    + "\r\n"+lr.eval_string("<sensitivity>")+"50</sensitivity>"
		    + "\r\n"+lr.eval_string("<enabled>")+"true</enabled>"
		    + "\r\n</motionSensitivity>\r\n";
	    lr.save_string(xmlBody,"xmlbody");
	    try {
		lr.save_string(DigestUtil.genDigist(""+lr.eval_string("<pwd>")+"",""+lr.eval_string("<userid>")+"",""+lr.eval_string("<pwd>")+"",""+lr.eval_string("<method>")+"",""+lr.eval_string("<uri>")+""),"resp");
	...
	private void doorOpenedRemindRequest(String[] s){
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/appremote/"+lr.eval_string("<qid>")+"/device/doorOpenedRemind","uri");
	    lr.save_string("GET","method");
	    xmlBody = "Body=";
	    lr.save_string(xmlBody,"xmlbody");
	    //System.out.println(xmlBody);
	...
	private void getSettingInfoRequest(String[] s){
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/app/device/"+lr.eval_string("<qid>")+"/getSettingInfo.app","uri");
	    lr.save_string("GET","method");
	    xmlBody = "Body=";
	    lr.save_string(xmlBody,"xmlbody");
	    try {
	    ...
	    lr.save_string("POST","method");
	    try {
		lr.save_string(HashUtil.getEncCheckSum("REMOVE_SHARE_DEVICE",""+lr.eval_string("<userID>")+"",""+lr.eval_string("<compID>")+""),"checks");
	    } catch ( Exception err ) {
		lr.message("An exception occurred: " + err.toString());
	    }
	    xmlBody = "Body=<?xml version=\"1.0\"?>"
