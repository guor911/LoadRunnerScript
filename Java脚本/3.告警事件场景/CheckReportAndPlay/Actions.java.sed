...
public class Actions
{
	...
	private String checkEventRequest(String[] s){
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/app/event/"+lr.eval_string("<eventid>")+"/getGifUrl.app","uri");//ID为每个事件记录唯一标识
	    ...
	    lr.save_string(s[0],"userid");
	    lr.save_string(s[1],"pwd");
	    lr.save_string("/sklcloud/app/event/"+lr.eval_string("<eventid>")+"/getPlaybackUrl.app","uri");//ID为每个事件记录唯一标识
	    lr.save_string("GET","method");
	    xmlBody = "Body=";
	    lr.save_string(xmlBody,"xmlbody");
	    try {
