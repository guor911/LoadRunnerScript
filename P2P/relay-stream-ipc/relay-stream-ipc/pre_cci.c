# 1 "c:\\users\\yueqiu\\desktop\\\275\305\261\276\\relay-stream-ipc\\relay-stream-ipc\\\\combined_relay-stream-ipc.c"
...
# 1 "c:\\users\\yueqiu\\desktop\\\275\305\261\276\\relay-stream-ipc\\relay-stream-ipc\\\\combined_relay-stream-ipc.c" 2

# 1 "vuser_init.c" 1
# 1 "D:\\Program Files\\HP\\LoadRunner\\include/lrs.h" 1
 



 
# 1 "D:\\Program Files\\HP\\LoadRunner\\include/lrs_err.h" 1
...
# 6 "D:\\Program Files\\HP\\LoadRunner\\include/lrs.h" 2
...
int len = 0;
char *ipcPort;
const char *RHost = "RemoteHost=192.168.139.213:8300";
...
vuser_init()
{
	...
	LrsCreateSocket("ipcsocket0", "TCP", RHost, "0");
	
	ipcPort = LrsGetSocketAttrib("ipcsocket0", LOCAL_PORT );

	itoa(ipcPort, port, 10);
	lr_save_string(port,"remoteport1");

	lr_start_transaction("ipc_login");
	{
		if(LrsSend("ipcsocket0", "ipc_login", "0")) return -1;
		if(custom_lrs_receive("ipcsocket0", "ipc_login_rsp", "0")) return -1;
	}
	lr_end_transaction("ipc_login", 2);

	LrsSetSendTimeout(60,0);
	LrsSetRecvTimeout(0,10000);
	LrsSetRecvTimeout2(0,10000);
    return 0;
}

# 2 "c:\\users\\yueqiu\\desktop\\\275\305\261\276\\relay-stream-ipc\\relay-stream-ipc\\\\combined_relay-stream-ipc.c" 2
...
{	
	...
	do{	
		...
		while(num++ < cnt) 
		{
			if(LrsSend("ipcsocket0", "ipc_relay_mutli""1080", "0")) return -1; 
		...
		LrsReceive("ipcsocket0", "ipc_relay_mutli_rsp""1080", "0");

		lr_end_transaction("ipc_relay", 2);
...
# 3 "c:\\users\\yueqiu\\desktop\\\275\305\261\276\\relay-stream-ipc\\relay-stream-ipc\\\\combined_relay-stream-ipc.c" 2
...
vuser_end()
{
	LrsCloseSocket("ipcsocket0");
    LrsCleanup();

    return 0;
}

# 4 "c:\\users\\yueqiu\\desktop\\\275\305\261\276\\relay-stream-ipc\\relay-stream-ipc\\\\combined_relay-stream-ipc.c" 2
