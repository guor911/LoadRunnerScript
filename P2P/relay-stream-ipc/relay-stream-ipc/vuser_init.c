...
vuser_init()
{
	...
	lrs_create_socket("ipcsocket0", "TCP", RHost, LrsLastArg);
	
	ipcPort = lrs_get_socket_attrib("ipcsocket0", LOCAL_PORT );
	...
	{
		if(lrs_send("ipcsocket0", "ipc_login", LrsLastArg)) return -1;
		if(custom_lrs_receive("ipcsocket0", "ipc_login_rsp", LrsLastArg)) return -1;
	}
	lr_end_transaction("ipc_login", LR_AUTO);
